...
CREATE TABLE `master_crop_variety` (
  `full_code` varchar(15) PRIMARY KEY COMMENT '대분류+품목+품종 코드 (예: 120101)',
  `item_code` varchar(6) NOT NULL COMMENT '대분류+품목 코드 (예: 1201)',
...
CREATE TABLE `map_region_weather_station` (
  `plor_cd` varchar(20) PRIMARY KEY COMMENT '원천 산지코드 (KAMIS)',
  `plor_nm` varchar(50),
  `j_sanji_cd` int COMMENT '직팜 내부 코드',
  `j_sanji_nm` varchar(50),
  `station_cd` varchar(10) COMMENT '기상관측소코드'
);
